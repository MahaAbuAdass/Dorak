...
class GetAllServicesViewModel (context: Context) : ViewModel()  {

    private val retrofitBuilder = RetrofitBuilder(context)

    private val _servicesResponse = MutableLiveData<List<ServicesResponse>>()
    val servicesResponse: LiveData<List<ServicesResponse>> = _servicesResponse
    ...
    suspend fun getAllServices(){
        viewModelScope.launch {
            try {
                val response = retrofitBuilder.getServices()
                _servicesResponse.postValue(response)
