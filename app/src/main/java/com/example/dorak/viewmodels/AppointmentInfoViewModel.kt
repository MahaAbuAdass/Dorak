...
class AppointmentInfoViewModel (context: Context) : ViewModel()  {

    private val retrofitBuilder = RetrofitBuilder(context)

    private val _appointmentResponse = MutableLiveData<AppointmentResponse>()
    val appointmentResponse: LiveData<AppointmentResponse> = _appointmentResponse
    ...
    suspend fun getAppoitmentInfo(appNo: String){
        viewModelScope.launch {
            try {
                val response = retrofitBuilder.appointmentInfo(appNo)
                _appointmentResponse.postValue(response)
