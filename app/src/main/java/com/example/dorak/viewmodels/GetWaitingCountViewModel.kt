...
class GetWaitingCountViewModel(context: Context) : ViewModel()  {

    private val retrofitBuilder = RetrofitBuilder(context)

    private val _getWaitingCountResponse = MutableLiveData<WaitingCountResponse>()
    val getWaitingCountResponse: LiveData<WaitingCountResponse> = _getWaitingCountResponse
    ...
    suspend fun getWaitingCount(branchCode : String , qID: String){
        viewModelScope.launch {
            try {
                val response = retrofitBuilder.getWaitingCount(branchCode, qID)
                _getWaitingCountResponse.postValue(response)
